...
public class JazzySpellChecker implements SpellCheckListener {
	...
	static {
		...
		File dict = new File("/home/sp/dev/tool/eclipse/gae_workspace/AdScraperFiltering/resources/de_DEx.dic");
		try {
	...
	public static void main(String[] args) {
		...
		if (args.length > 0) {
			try {
				words = FileUtils.readFileToString(new File(args[0]));
			} catch (IOException e1) {
				System.out.println("probably invalid file: " + args[0]);
				throw new RuntimeException(e1);
			}
		}
		else
			words = "sag mir nicht, wer ich bin, he!"; 
		JazzySpellChecker sc = new JazzySpellChecker();
		sc.initialize();
		for (Entry<String, Integer> e : getErrorFrequencies(sc.getMisspelledWords(words))) {
			System.out.println(String.format("%d %s", e.getValue(), e.getKey()));
		}	
	}
	
	private static List<Entry<String, Integer>> getErrorFrequencies(List<String> errors)
	{
		return Dictionary.sortByFrequency(Dictionary.aggregateByFrequency(errors));
	}
}